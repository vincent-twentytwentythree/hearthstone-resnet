...
mean_episode_return_buf = {p:deque(maxlen=100) for p in ['landlord', 'second_hand']}
...
def train(flags):
    ...
    for device in device_iterator:
        _free_queue = {'landlord': ctx.SimpleQueue(), 'second_hand': ctx.SimpleQueue(),}
        _full_queue = {'landlord': ctx.SimpleQueue(), 'second_hand': ctx.SimpleQueue(),}
    ...
    position_frames = {'landlord':0, 'second_hand':0}

    # Load models if any
    for k in ["landlord", "second_hand"]:
        checkpointpath = os.path.expandvars(
            os.path.expanduser('%s/%s/%s' % (flags.savedir, flags.xpid, k + '_model.tar'))) # todo
    ...
    for device in device_iterator:
        get_data_device_locks[device] = {'landlord': threading.Lock(), 'second_hand': threading.Lock()}
    learn_model_position_locks = {'landlord': threading.Lock(), 'second_hand': threading.Lock()}
        ...
        for i in range(flags.num_threads):
            for position in ["landlord", "second_hand"]:
                thread = threading.Thread(
                    target=batch_and_learn, name='batch-and-learn-%d' % i, args=(i,device,position,get_data_device_locks[device][position],learn_model_position_locks[position]))
                thread.start()
    ...
    def checkpoint(frames):
        ...
        for k in ["landlord", "second_hand"]:
            ...
            torch.save({
                'model_state_dict': {k: _models[k].state_dict()},
                'optimizer_state_dict': {k: optimizers[k].state_dict()},
                "stats": stats,
                'flags': vars(flags),
                'frames': frames,
    ...
    try:
        ...
        while frames < flags.total_frames:
            ...
            log.info('After %i (L:%i S:%i) frames: @ %.1f fps (avg@ %.1f fps) (L:%.1f S:%.1f) Stats:\n%s',
                     frames,
